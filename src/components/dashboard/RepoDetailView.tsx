...
import { ArrowLeft, GitBranch, Shield, AlertTriangle, ExternalLink, Users, Clock } from "lucide-react";
import { StatCard } from "./StatCard";

export function RepoDetailView() {
    const { state, setState } = useGitHubApp();
    const repo = state.repos.find(r => r.name === state.selectedRepoName);
    const alerts = state.alerts.filter(a => a.repo === state.selectedRepoName);
                ...
                <div>
                    <div className="flex items-center gap-3">
                        ...
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                            repo.visibility === 'public' ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning'
                        }`}>
                            {repo.visibility}
                        </span>
                    </div>
